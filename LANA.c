        ...
        for (unsigned x = 2; x < width - 2; ++x) {
            ...
            for (int i = -2; i <= 2; ++i) {
                for (int j = -2; j <= 2; ++j) {
                    ...
                    unsigned char r = pixel[0];
                    unsigned char g = pixel[1];
                    unsigned char b = pixel[2];int kernel_value_x = sobel_x[i + 2][j + 2];
                    int kernel_value_y = sobel_y[i + 2][j + 2];
...
void bfs(unsigned char* output, unsigned char* image, unsigned width, unsigned height, unsigned x, unsigned y, bool* visited) {
    unsigned char red = (50 + rand()) % 206+10;
    unsigned char green = (100 + rand()) % 156 + 55;
    ...
    while (left < right) {
        ...
        for (int i = -1; i <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                ...
                if (!(newx >= 2 && newy >= 2 && newx < width - 2 && newy < height - 2)) continue;
...
void segmentation(unsigned char* output, unsigned char* image, unsigned width, unsigned height) {
    ...
    srand(8);
    const char* input_filename = "jioujihkikoj.png";
    const char* output_filename = "oijedijwdiiop.png";
